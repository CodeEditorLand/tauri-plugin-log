...
///
/// It is very similar to the [`log::Level`], but serializes to unsigned ints
/// instead of strings.
...
pub enum LogTarget {
	...
	///
	/// This requires the webview to subscribe to log events, via this plugins
...
impl Builder {
	...
	///
	/// Note: This does not modify the directory logs go into. For that refer to
	/// `LogTarget::Folder`.
	///
	/// # Examples
	///
			...
			.setup(move |app_handle| {
				...
				for target in &self.targets {
					self.dispatch = self.dispatch.chain(match target {
						LogTarget::Stdout => {
							fern::Output::from(std::io::stdout())
						},
						LogTarget::Stderr => {
							fern::Output::from(std::io::stderr())
						},
			...
			match rotation_strategy {
				RotationStrategy::KeepAll => {
					...
					if to.is_file() {
						...
						// highly unlikely but defensively handle anyway by
						// adding .bak to filename
